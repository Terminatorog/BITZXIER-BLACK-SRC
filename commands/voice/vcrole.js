...
module.exports = {
    ...
    category: 'voice',
    premium: true,
    subcommand : ['humans set','bots set','view','reset'],
                    ...
                    .then(async (data) => {
                        ...
                        if (!data) {
                            ...
                            const saiop = client.util.embed()
                                .setColor(client.color)
                                .setDescription(
                                    `Great news! Your server is now configured perfectly. Feel free to use vcrole module and enjoy using your server hassle-free! <a:rockets:1376304217278648530>`
                                )
...
if (option.toLowerCase() === 'set') {
    ...
    if (role && !role.permissions.has('Administrator','KICK_MEMBERS','BanMembers','ManageChannels','ManageGuild','MENTION_EVERYONE','ManageRoles','ManageWebhooks')) {
        await client.db.set(`vcroles_${message.guild.id}`, {
            vcrole : role.id
        })
        message.channel.send({
